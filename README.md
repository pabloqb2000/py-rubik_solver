...
- For statistics:
    ...
    ```python stats.py```
## Requirements
To use this program you need to install _python 3.8.10_ or later (all though it will probably work on _python 3.7_)
...
## Implementation
This project is separated in different files, each implementing a different functionality. The content and functionality of each of this files is the following:
### configs.py
This file contains general configuration paremeters mostly related to the visual representation of the cube:
- The default colors
- The number of fps
...
- ```rotate(axis, n=1)```: this has the same effect as using ```move``` with _"UU", "FF", "RR"_ but this moves are never recorded.
...
- ```copy()```: creates a _deep_copy_ of the cube. The copy is completly independent of the original cube.
### cube_3d.py
...
### simple_solver.py
This is the first solving algorithm implemented, it's the usual _beginer_ algorithm for anyone learing how to solve the rubiks cube. It's implemented on a really naive way and it's far from optimal in terms of the number of steps of the solution. It was just a proof of concept and my goal is to implement a better, more efficient version of this class in the future.

In my personal computer this algorithm takes ```1.78 ms``` on average to compute a solution, and the solutions have ```205.6``` steps on average. Again this results are far from good, but this was just a proof of concept.
